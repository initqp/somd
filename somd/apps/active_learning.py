...
class ACTIVELEARNING(_mdapps.simulations.STAGEDSIMULATION):
    ...
    def __check_learning_parameters(self) -> None:
        ...
        if 'initial_potential_files' in param.keys() and self.n_iter == 0:
            if len(param['initial_potential_files']) != param['n_potentials']:
                message = (
                    'Number of the initial potential files should '
        ...
        if (
            ...
            message = (
                'Value of the "min_new_structures_per_iter" key '
        ...
        if param['max_md_steps_per_iter'] < param['trajectory_interval']:
            message = (
                'Value of the "trajectory_interval" should be '
                + 'smaller than value of the "max_md_steps_per_iter" key!'
            )
            raise RuntimeError(message)
        if 'initial_training_set' in param.keys() and self.n_iter != 0:
            sets = self.root['/iteration_data/0'].attrs['training_sets'][0]
            param['initial_training_set'] = sets
            message = (
                'The key "initial_training_set" will be ignored '
                + 'since the simulation is being restarted.'
            )
            _mdutils.warning.warn(message)
        if 'initial_potential_files' in param.keys() and self.n_iter != 0:
            message = (
                'The key "initial_potential_files" will be ignored '
                + 'since the simulation is being restarted.'
            )
            _mdutils.warning.warn(message)
    ...
    def __initialize_iteration_data_group(self, h5_path: str) -> None:
        ...
        self._create_dataset(
            h5_path + '/n_visited_structures', (1,), (1,), int
        )
        h5_group['n_visited_structures'][0] = 0
        self._create_dataset(
            h5_path + '/n_accurate_structures', (1,), (1,), int
        )
        h5_group['n_accurate_structures'][0] = 0
        self._create_dataset(
            h5_path + '/n_candidate_structures', (1,), (1,), int
        )
        h5_group['n_candidate_structures'][0] = 0
        self._create_dataset(
            h5_path + '/n_accepted_structures', (1,), (1,), int
        )
        h5_group['n_accepted_structures'][0] = 0
        self._create_dataset(h5_path + '/n_failed_structures', (1,), (1,), int)
        h5_group['n_failed_structures'][0] = 0
        self._create_dataset(
            h5_path + '/n_untrained_structures', (1,), (1,), int
        )
        h5_group['n_untrained_structures'][0] = 0
        self._create_dataset(
            h5_path + '/energy_shift', (1,), (1,), _np.double, 'kJ/mol'
        )
        h5_group['energy_shift'][0] = param['energy_shift']
        self._create_dataset(
            h5_path + '/min_new_structures_per_iter', (1,), (1,), int
        )
        h5_group['min_new_structures_per_iter'][0] = param[
            'min_new_structures_per_iter'
        ]
        self._create_dataset(
            h5_path + '/max_new_structures_per_iter', (1,), (1,), int
        )
        h5_group['max_new_structures_per_iter'][0] = param[
            'max_new_structures_per_iter'
        ]
        self._create_dataset(
            h5_path + '/max_force_msd', (1,), (1,), _np.double, 'kJ/mol/nm'
        )
        h5_group['max_force_msd'][0] = 0.0
        self._create_dataset(
            h5_path + '/force_msd_lower_limit',
            (1,),
            (1,),
            _np.double,
            'kJ/mol/nm',
        ...
        self._create_dataset(
            h5_path + '/candidate_structure_indices', (0,), (None,), int
        )
        self._create_dataset(
            h5_path + '/accepted_structure_indices', (0,), (None,), int
        )
        self._create_dataset(
            h5_path + '/force_msd', (0,), (None,), _np.double, 'kJ/mol/nm'
        )
        self._create_dataset(
            h5_path + '/accepted_structure_energies',
            (0,),
            (None,),
            _np.double,
            'kJ/mol',
    ...
    def __update_neps(self, n_iter: int) -> tuple:
        ...
        potential_files = [
            working_dir + nep_path.format(i) for i in range(0, n_potentials)
        ]
        for i in range(0, n_potentials):
            p = _NEP(
                range(0, self.system.n_atoms),
                potential_files[i],
                self.system.atomic_symbols,
                self.__use_tabulating,
            )
            self.__neps.append(p)
        # Determine which NEP to use.
        try:
            files = [
                working_dir + '/potential_{:d}/loss.out'.format(i)
                for i in range(0, n_potentials)
            ]
    ...
    def __propagate(self, active_potential_index: int) -> None:
        ...
        potential_indices = [
            ...
            if i not in self.__reference_potentials
        ]
        with self._set_up_simulation(potential_indices, nep) as simulation:
            ...
            force_msd_limits = [
                param['msd_lower_limit'],
                param['msd_upper_limit'],
            ]
            n_epochs = (
                param['max_md_steps_per_iter'] // param['trajectory_interval']
            )
            for i in range(0, param['max_md_runs_per_iter']):
                simulation.restart_from(
    ...
    def __perform_pretraining(self) -> None:
        ...
        if not self.root[h5_path].attrs['initialized']:
            ...
            if 'initial_potential_files' in param.keys():
                ...
                # Write the data sets.
                _apputils.nep.cat_exyz(
                    [param['initial_training_set']], working_dir + '/train.xyz'
                )
                _apputils.nep.cat_exyz(
                    [param['initial_testing_set']], working_dir + '/test.xyz'
                )
                # Update the log file.
    ...
    def __perform_training(self) -> None:
        ...
        if not h5_group.attrs['initialized']:
            self.__initialize_iteration_data_group(h5_path)
            h5_group.attrs['system_data'] = working_dir + '/system_data.csv'
            h5_group.attrs['visited_structures'] = (
        ...
        n_new_structures = (
            h5_group['n_accepted_structures'][0]
        ...
        if n_new_structures > param['min_new_structures_per_iter']:
            ...
            for j in range(0, param['n_potentials']):
                if not _os.path.islink('potential_{:d}'.format(j)):
                    _os.symlink(
                        _os.path.relpath(potential_dir_old.format(j)),
                        'potential_{:d}'.format(j),
                    )
            h5_group.attrs['training_sets'] = h5_group_old.attrs[
                'training_sets'
            ]
