...
static inline void _d_distance_d_r(
    double *positions,
    vector<int> indices,
    double value,
    double *deriv
)
{
    ...
    idx_2 = indices[1] * 3;
    for (i = 0; i < 3; i++) {
        deriv[i + 0] = (positions[idx_1 + i] - positions[idx_2 + i]) / value;
        deriv[i + 3] = (positions[idx_2 + i] - positions[idx_1 + i]) / value;
    }
}

// Calculate derivatives: d_angle / d_r.
static inline void _d_angle_d_r(
    double *positions,
    vector<int> indices,
    double value,
    double *deriv
)
{
    ...
    for (i = 0; i < 3; i++) {
        deriv[i + 0] = (
            (positions[idx_1 + i] - positions[idx_2 + i]) / r_12 * cos_angle -
            (positions[idx_3 + i] - positions[idx_2 + i]) / r_32
        ) / r_12 / sin_angle;
        deriv[i + 6] = (
...
static inline void _d_torsion_d_r(
    double *positions,
    vector<int> indices,
    double value,
    double *deriv
)
{
    ...
    for (i = 0; i < 3; i++) {
        deriv[i + 0] = r_32_len / r_12_cross_r_32_norm_2 * r_12_cross_r_32[i];
        deriv[i + 9] = r_32_len / r_32_cross_r_34_norm_2 * r_32_cross_r_34[i];
        deriv[i + 9] = deriv[i + 9] * -1.0;
    }
    for (i = 0; i < 3; i++) {
        deriv[i + 3] = (
            deriv[i + 0] * (r_12_dot_r_32 / r_32_len / r_32_len - 1.0) -
            deriv[i + 9] * (r_34_dot_r_32 / r_32_len / r_32_len)
        );
        deriv[i + 6] = (
            deriv[i + 9] * (r_34_dot_r_32 / r_32_len / r_32_len - 1.0) -
            deriv[i + 0] * (r_12_dot_r_32 / r_32_len / r_32_len)
        );
    }
}
...
void RATTLE::calculate_geometry_variables_derivatives(double *positions, int t)
{
    ...
    for (int i = 0; i < n_constraints; i++) {
        ...
        case 0:
            _d_distance_d_r(
                positions, indices[i], values[t][i], deriv[t][i].data()
            );
            break;
        case 1:
            _d_angle_d_r(
...
void RATTLE::rattle_constrain_q(
    double *positions,
    double *velocities,
    double *mass,
    double dt,
    ...
    for (int j = 0; j < max_cycles; j++) {
        ...
        ** at next time step. */
        ...
        for (int i = 0; i < n_constraints; i++) {
            ...
            ** go to next one */
            ...
            for (int l = 0; l < (int)(indices[i].size()); l++) {
                int idx = indices[i][l];
                velocities[idx * 3 + 0] -= (
                    0.5 * gamma * deriv[0][i][l * 3 + 0] / mass[idx]
                );
                velocities[idx * 3 + 1] -= (
                    0.5 * gamma * deriv[0][i][l * 3 + 1] / mass[idx]
                );
                velocities[idx * 3 + 2] -= (
                    0.5 * gamma * deriv[0][i][l * 3 + 2] / mass[idx]
                );
            }
            CONST_I_END:;
        }
        if (flag == 0) {
            ...
            string msg = (
                "RATTLE PART 1 DID NOT CONVERGE AFTER " +
                to_string(max_cycles) +
                " STEPS!"
            );
            cerr << msg << endl;
            if (die_on_fail)
...
void RATTLE::rattle_constrain_p(
    double *positions,
    double *velocities,
    double *mass,
    double dt,
    ...
    ** at next time step. */
    ...
    for (int j = 0; j < max_cycles; j++) {
        ...
        for (int i = 0; i < n_constraints; i++) {
            ...
            ** tolerance, go to next one */
            ...
            for (int l = 0; l < (int)(indices[i].size()); l++) {
                int idx = indices[i][l];
                velocities[idx * 3 + 0] -= (
                    0.5 * eta * deriv[1][i][l * 3 + 0] / mass[idx]
                );
                velocities[idx * 3 + 1] -= (
                    0.5 * eta * deriv[1][i][l * 3 + 1] / mass[idx]
                );
                velocities[idx * 3 + 2] -= (
                    0.5 * eta * deriv[1][i][l * 3 + 2] / mass[idx]
                );
            }
            CONST_I_END:;
        }
        if (flag == 0) {
            ...
            string msg = (
                "RATTLE PART 2 DID NOT CONVERGE AFTER " +
                to_string(max_cycles) +
                " STEPS!"
            );
            cerr << msg << endl;
