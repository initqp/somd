...
class MDSYSTEM(object):
    ...
    def __allocate(self) -> None:
        ...
        """
        self.__types = _np.zeros((self.n_atoms, 1), _np.int_)
    ...
    def update_potentials(self, indices: list = None) -> None:
        ...
        for i in l:
            self.__virial[:] += self.__potentials[i].virial
            self.__energy_potential += self.__potentials[i].energy_potential[0]
            self.__forces[self.__potentials[i].atom_list] += \
    ...
    def positions_wrapped(self) -> _np.ndarray:
        ...
        """
        s = _np.linalg.pinv(self.box.T).dot(self.positions.T)
        s = s % 1
